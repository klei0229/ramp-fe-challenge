...
**My Bug Fix:** Removed the fixed position style in the "RampInputSelect--dropdown-container" class of index.css
...
**My Bug Fix:** In src/components/InputCheckbox/index.tsx, the input component is not on screen because the class "RampInputCheckbox--input" has a display:'none' tag. To fix this, I commented out the display none, changed the opacity to zero and overlapped it onto the checkbox label'
...
**My Bug Fix:** In App.tsx, for the input select, we add an IF statement to catch when the variable newValue.id is an empty string which represents to get data transaction from all employees. So instead of going into the loadTransactionByEmployee function, we execute loadAllTransactions()
...
**My Bug Fix:** In the setPaginatedTransaction function, we need to include the the array of transactions on the page before the view more button is pressed, onto the object that is being returned 
...
**My Bug Fix** changed the isLoading prop from isLoading which toggles true/false when loadAlltransactions runs. In its place I used employeeUtils.loading which checks when employees is loading
...
**My Bug Fix** for the hooks usePaginatedTransactions and useTransctionsByEmployee, I substituted fetchWithCache with fetchWithoutCache so that on data retrival, the component will grab the updated values instead of the cached values
